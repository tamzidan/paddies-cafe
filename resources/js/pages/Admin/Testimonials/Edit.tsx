...
interface Testimonial { 
    ...
    rating: number; avatar: string; is_active: boolean;
}
...
export default function TestimonialEdit({ testimonial }: { testimonial: Testimonial }) {
    ...
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        { title: 'Edit Testimoni', href: route('admin.testimonials.edit', testimonial.id) },
    ];
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="max-w-2xl mx-auto bg-card p-6 rounded-lg shadow-sm">
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    </div>                    {/* Perbedaannya hanya di field avatar, kita tampilkan gambar yang sudah ada */}
                    <div>
                        <label className="block text-sm font-medium mb-1">Avatar</label>
                        {testimonial.avatar && (
                            <div className="mb-2">
                                <img src={`/storage/${testimonial.avatar}`} alt={testimonial.name} className="w-24 h-24 object-cover rounded-full border" />
                                <p className="text-xs text-gray-500 mt-1">Ganti file untuk mengubah avatar.</p>
                            </div>
                        )}
